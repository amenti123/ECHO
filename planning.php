...
// ✅ Ensure new column hh_count exists (for household targets) in indicator tables
        ...
        // Normalize indicator target columns to DECIMAL (fixes 'indicator not saving' due to missing/INT columns; keeps existing features)
        foreach (['impact_indicators','outcome_indicators','output_indicators'] as $t) {
            try { $pdo->exec("ALTER TABLE {$t} ADD COLUMN target_total DECIMAL(15,2) DEFAULT 0"); } catch (Throwable $e) {}
            try { $pdo->exec("ALTER TABLE {$t} MODIFY COLUMN total_target DECIMAL(15,2) NULL DEFAULT 0"); } catch (Throwable $e) {}
            try { $pdo->exec("ALTER TABLE {$t} MODIFY COLUMN target_total DECIMAL(15,2) NULL DEFAULT 0"); } catch (Throwable $e) {}
        }
...
// ------------------------------------------------------
// Indicator unit normalization helpers (defined early for POST handler)
// ------------------------------------------------------
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $project_id = (int)($_POST['project_id'] ?? ($_SESSION['selected_project_id'] ?? 0));
    $anchor = 'projects';
    $msg = '';
    $err = '';
...
.scroll-hint {
    ...
    margin-bottom: 8px;
        ...
        $format     = strtolower($_GET['format'] ?? 'pdf'); // default to PDF for real exports
        $project_id = (int)($_GET['project_id'] ?? ($_GET['p'] ?? 0));
        ...
        // Normalize section names coming from UI
        $section = trim($section);
        $sectionMap = [
            'impact'  => 'impact_indicators',
            'outcome' => 'outcome_indicators',
            'output'  => 'output_indicators',
        ];
        if (isset($sectionMap[$section])) {
            $section = $sectionMap[$section];
        }

        // Fetch project header info for export title
        $projectTitle = 'Project ' . $project_id;
        try {
            $pStmt = $pdo->prepare("SELECT title, project_code FROM projects WHERE id = ?");
            $pStmt->execute([$project_id]);
            if ($pRow = $pStmt->fetch(PDO::FETCH_ASSOC)) {
                $projectTitle = trim(($pRow['title'] ?? '') . (!empty($pRow['project_code']) ? " ({$pRow['project_code']})" : '')) ?: $projectTitle;
            }
        } catch (Throwable $e) { /* ignore */ }

        // Helper to render HTML table
        $renderTable = function(array $headers, array $rows, string $title): string {
            $h = '<h3 style="margin:0 0 10px 0;">' . htmlspecialchars($title, ENT_QUOTES, 'UTF-8') . '</h3>';
            $h .= '<table border="1" cellpadding="6" cellspacing="0" style="border-collapse:collapse;width:100%;font-size:12px;">';
            $h .= '<thead><tr>';
            foreach ($headers as $th) {
                $h .= '<th style="background:#f5f5f5;text-align:left;">' . htmlspecialchars($th, ENT_QUOTES, 'UTF-8') . '</th>';
            }
            $h .= '</tr></thead><tbody>';
            foreach ($rows as $r) {
                $h .= '<tr>';
                foreach ($r as $cell) {
                    $h .= '<td>' . htmlspecialchars((string)$cell, ENT_QUOTES, 'UTF-8') . '</td>';
                }
                $h .= '</tr>';
            }
            $h .= '</tbody></table>';
        ...
        if ($section === 'all' || $section === 'combined') {
            $sectionLabel = 'all';
            $sectionsToExport = ['results', 'impact_indicators', 'outcome_indicators', 'output_indicators', 'activities'];
        } else {
        ...
        foreach ($sectionsToExport as $sec) {
            $headers = [];
            $rows    = [];

            switch ($sec) {
                case 'results':
                    $headers = ['Type', 'Title', 'Description'];
                    $stmt = $pdo->prepare("SELECT chain_type, title, description FROM results_chain WHERE project_id = ? ORDER BY id ASC");
                    $stmt->execute([$project_id]);
                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        $rows[] = [
                            ucfirst($row['chain_type']),
                            $row['title'],
                            $row['description']
                        ];
                    }
                    $htmlParts[] = $renderTable($headers, $rows, "Results Chain (Impact / Outcome / Output) - {$projectTitle}");
                    break;

                case 'impact_indicators':
                case 'outcome_indicators':
                case 'output_indicators':
                case 'indicators':
                    $headers = ['Type', 'Code', 'Indicator', 'Unit', 'Total Target', 'Disaggregation'];
                    $tables = [];
                    if ($sec === 'impact_indicators') $tables = [['impact_indicators', 'Impact']];
                    elseif ($sec === 'outcome_indicators') $tables = [['outcome_indicators', 'Outcome']];
                    elseif ($sec === 'output_indicators') $tables = [['output_indicators', 'Output']];
                    else $tables = [['impact_indicators', 'Impact'], ['outcome_indicators', 'Outcome'], ['output_indicators', 'Output']];

                    foreach ($tables as [$tbl, $label]) {
                        $stmt = $pdo->prepare("SELECT indicator_code, indicator_name, unit_type, total_target, disaggregation FROM {$tbl} WHERE project_id = ? ORDER BY id ASC");
                        $stmt->execute([$project_id]);
                        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                            $rows[] = [
                                $label,
                    ...
                    $htmlParts[] = $renderTable($headers, $rows, "Indicators - {$projectTitle}");
                    break;

                case 'activities':
                    $headers = ['Code', 'Activity', 'Sector', 'Unit', 'Target', 'Start', 'End'];
                    $stmt = $pdo->prepare("SELECT activity_code, activity_description, sector, unit_of_measurement, target, start_date, end_date FROM activities WHERE project_id = ? ORDER BY id ASC");
                    $stmt->execute([$project_id]);
                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        $rows[] = [
                            $row['activity_code'],
                    ...
                    $htmlParts[] = $renderTable($headers, $rows, "Planned Activities - {$projectTitle}");
                    break;
            ...
            // For CSV: if exporting single section, set header/rows
            if (count($sectionsToExport) === 1) {
                $csvHeader = $headers;
                $csvRows   = $rows;
            }
        }
        ...
        if ($format === 'csv') {
            header('Content-Type: text/csv; charset=utf-8');
            header('Content-Disposition: attachment; filename="planning_' . $sectionLabel . '_project_' . $project_id . '.csv"');
        ...
        if ($format === 'excel' || $format === 'xls') {
            $html = '<html><head><meta charset="UTF-8"></head><body>' . implode('<br><br>', $htmlParts) . '</body></html>';
            header('Content-Type: application/vnd.ms-excel; charset=utf-8');
            header('Content-Disposition: attachment; filename="planning_' . $sectionLabel . '_project_' . $project_id . '.xls"');
            echo $html;
            exit;
        }

        if ($format === 'html' || $format === 'print') {
            $html = '<html><head><meta charset="UTF-8"><title>Planning Export</title></head><body>' . implode('<br><br>', $htmlParts) . '</body></html>';
        ...
        // Default: real PDF
        $html = '<html><head><meta charset="UTF-8"><style>body{font-family:DejaVu Sans, Arial, sans-serif;}</style></head><body>'
              . implode('<div style="page-break-after:always;"></div>', $htmlParts)
              . '</body></html>';

        if (function_exists('hrs_export_pdf_from_html')) {
            hrs_export_pdf_from_html($html, 'planning_' . $sectionLabel . '_project_' . $project_id . '.pdf');
            exit;
        }

        // If no PDF library is available, fall back to HTML
        header('Content-Type: text/html; charset=utf-8');
        echo $html;
        exit;
    }
...
<div class="card">
    <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px; padding: 20px;">
        ...
        <div class="export-options">
            <button class="btn btn-primary" onclick="exportComprehensiveView('all', 'pdf')">
                🖨️ Print / PDF
            </button>
...
<div class="card" id="comprehensive-view">
    <div class="section-toggle" onclick="toggleSection('comprehensive-view-content')">
        ...
        <div style="margin-left: auto; display: flex; gap: 5px;">
            ...
            <button class="btn btn-sm btn-info" onclick="exportComprehensiveView('all', 'pdf')" title="Print">🖨️ Print</button>
        </div>
    </div>
...
<div class="card" id="results">
    <div class="section-toggle" onclick="toggleSection('add-result')">
        ...
        <div style="margin-left: auto; display: flex; gap: 5px;">
            <button class="btn btn-sm btn-success" onclick="exportComprehensiveView('all', 'pdf')" title="Print this section">🖨️ Print</button>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function initIndicatorForm(form) {
        ...
        function updateMode() {
            ...
            var isPersonSADD = isPersonUnit(unit) && inputMode === 'sadd';
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function initIndicatorForm(form) {
        ...
        function updateMode() {
            ...
            // Enable/disable all SADD + PWD + HH for non-person units
...
function exportComprehensiveView(section, format = 'pdf') {
            ...
            const url = `planning.php?action=export_section&project_id=${encodeURIComponent(projectId)}&section=${encodeURIComponent(section)}&format=${encodeURIComponent(format)}`;

            // Use same-tab navigation to avoid popup blockers (works reliably for downloads)
            window.location.href = url;
        }
